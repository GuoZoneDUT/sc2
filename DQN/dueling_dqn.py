    ...
    def build_net(self):
        ...
        def build_layers(s,c_names,):
            with tf.variable_scope("conv_1"):
                ...
                h=tf.nn.conv2d(s, W,strides=[1,1,1,1],padding='SAME')+b
                conv_1 = tf.nn.relu(h)
            ...
            if self.dueling:
                with tf.variable_scope("Value"):
                    W = tf.get_variable('Value_w',[128,1],initializer=w_initializer,collections=c_names)
                    b = tf.get_variable('Value_b',[1,1],initializer=b_initializer,collections=c_names)
                    self.V = tf.matmul(fc_1,W) + b

                with tf.variable_scope("Advantage"):
                    W = tf.get_variable('Advantage_W',[128,4],initializer=w_initializer,collections=c_names)
                    b = tf.get_variable('Advantage_B',[1,4],initializer=b_initializer,collections=c_names)
                    self.A = tf.matmul(fc_1,W)+ b
        ...
        with tf.variable_scope("q_eval"):
            ...
            with tf.name_scope("input"):
                ...
                self.q_eval = build_layers(simg,c_names)
        ...
        with tf.variable_scope("q_target"):
            ...
            self.q_next = build_layers(s_img,c_names)
